...
- **联网搜索** 🔥：集成 Tavily API，AI 自主决定搜索内容，获取实时网络信息
- **Function Calling** 🔥：基于 LangChain Tools 实现结构化函数调用，支持参数验证和类型约束
- **AI 计划生成**：基于用户目标 + 搜索结果自动生成 3-5 步的执行计划，步骤具体、可操作
...
- **Tavily Search API**：联网搜索工具，获取实时网络信息
- **DeepAgents**：创建支持 Function Calling 的智能体
...
3. **联网搜索与 Function Calling**
   - 使用 LangChain `tool()` 封装 Tavily 搜索 API
   - 通过 Zod Schema 定义函数参数和类型约束
   - AI 自主决定搜索内容和时机，实现智能检索
   - 搜索结果作为上下文输入给计划生成节点
...
- **联网搜索能力**：AI 自主决定搜索内容，获取实时信息，避免知识截止问题
- **Function Calling 实践**：掌握 LangChain Tools 的实现方式和最佳实践
...
- Function Calling 实现：掌握 LangChain Tools 的定义和调用
- 联网搜索集成：学习 Tavily API 和实时信息检索
- TypeScript 类型安全开发
- 流式响应（SSE）实现
- 人类参与式 AI 应用设计

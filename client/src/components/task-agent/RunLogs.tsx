...
function StreamPreview({ text }: { text: string }) {
  ...
  return (
    <div className="pl-8 border-l-2 border-dashed border-gray-200 dark:border-gray-700 ml-2.5">
  ...
  return (
    <div className="space-y-6">
      {entries.map(([node, state]) => {
        ...
        const tokenText = tokenLog.get(node) ?? "";
        ...
        return (
          <div key={node}>
            ...
            {node === "validate" && !state.message && <StreamPreview text={tokenText} />}
            ...
            {node === "search" && !state.searchResults && <StreamPreview text={tokenText} />}
            ...
            {node === "plan" && !state.steps && <StreamPreview text={tokenText} />}
            ...
            {node === "execute" && !state.results && <StreamPreview text={tokenText} />}
            {node === "finalize" && state.message && (
              <div className="pl-8 border-l-2 border-gray-200 dark:border-gray-700 ml-2.5">
                <p className="text-sm leading-7 text-muted-foreground whitespace-pre-wrap">
                  {state.message}
                </p>
            ...
            {node === "finalize" && !state.message && <StreamPreview text={tokenText} />}
          </div>
        );
      })}
    </div>
  ...
  return (
    <Card className="mt-6 shadow-sm">
      <CardContent className="p-6 space-y-6">
        ...
        <LogRenderer log={log} tokenLog={tokenLog} />
